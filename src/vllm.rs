    ...
    fn tok_to_str(&mut self, tok: Token) -> String {
        self.dec_cache
            ...
            .or_insert_with(|| {
                Python::attach(|py| {
                    self.tokenizer
                        .call_method1(py, intern!(py, "decode"), (vec![tok],))
                        .unwrap()
                        .extract(py)
                        .unwrap()
                })
...
impl<'a> VllmSession<'a> {
    pub fn new(model: &'a VllmModel, pfx: &str) -> VllmSession<'a> {
        let pfx = model.str_to_tokens(pfx);
        VllmSession {
            model,
...
impl<'a> Session<'a> for VllmSession<'a> {
    ...
    fn predict_str(&mut self, text: &str, top_p: Option<f32>) -> Vec<(Token, f64)> {
        let toks = self.model.str_to_tokens(text);
        self.predict(&toks, top_p)
    }

    fn predict(&mut self, toks: &[Token], top_p: Option<f32>) -> Vec<(Token, f64)> {
        Python::attach(|py| -> anyhow::Result<Vec<(Token, f64)>> {
            ...
            let prompt_tokens: Vec<Token> = self.pfx.iter().chain(toks.iter()).copied().collect();

            input_dict.set_item("prompt_token_ids", prompt_tokens)?;
